...
pub fn cad_boolean_fuse(shape1_id: String, shape2_id: String) -> Result<ShapeResult, String> {
    debug!("Boolean fuse: {} + {}", shape1_id, shape2_id);
    ...
    debug!(
        "Boolean fuse result: {} faces, {} solids (id: {})",
        analysis.num_faces, analysis.num_solids, id
    );
...
pub fn cad_simplify(
    ...
    // Warn if faces reduced significantly (possible over-simplification)
    if analysis.num_faces < before.num_faces / 2 {
        warn!(
            "Simplify: face count reduced from {} to {} - possible over-simplification",
    ...
    let id = store_shape(result)?;
    debug!(
        "Simplified shape: {} -> {} faces (id: {})",
        before.num_faces, analysis.num_faces, id
    );
