...
import {
  useLayoutActions,
  useShowModellerLeft,
  useShowModellerRight,
} from "@/core/stores/layout-store"
import { useModellerStore, useSelectedIds } from "@/core/stores/modeller"
import { useCurrentProject, useIsProjectLoading } from "@/core/stores/project-store"
import { ScenePanel } from "@/editors/space_outliner/ED_outliner_main"
import { PropertiesPanel } from "@/editors/space_properties/ED_props_main"
import { HistoryPanel } from "@/editors/space_timeline/ED_timeline_history"
import { PanelErrorBoundary, ViewerErrorBoundary } from "@/interface/common"
import { ImportDXFDialog, ImportIFCDialog } from "@/interface/dialogs"
import { useCADOperationHotkeys, useGlobalHotkeyHandler } from "@/lib/hooks"
import { useCAD } from "@/lib/hooks/use-kernel"
import { CADOperationsProvider } from "@/operators/context"
import { BoxLoader, ChuteCreator, CreatePanel, TransitionCreator } from "@/operators/create"
import { Viewport3D } from "@/render/RE_viewport3d"
import { CameraAnimationPanel, ViewportSettingsPanel } from "./panels"
 ...
 *
 * Note: This component must be rendered inside CADOperationsProvider.
 * During hot reload, React may attempt to render before provider is ready,
 * which will cause an error that will be caught by ErrorBoundary.
 */
...
function useKeyboardShortcuts(onToggleLeftPanel?: () => void) {
  ...
  useEffect(() => {
    ...
    try {
      unlistenPromise = listen<string>("menu-event", (event) => {
      ...
      // During hot reload, listen() might fail - this is expected
      if (import.meta.env.DEV) {
        console.warn("[Tauri] Failed to register menu-event listener (hot reload):", error)
      }
      return
    }

    return () => {
      // Cleanup listener - handle errors gracefully during hot reload
      unlistenPromise
        ?.then((fn) => {
          try {
            fn()
          } catch (error) {
            // Callback might already be cleaned up during hot reload
            if (import.meta.env.DEV) {
              console.warn("[Tauri] Failed to unlisten menu-event (hot reload):", error)
            }
          }
        })
        .catch((error) => {
          // Promise might be rejected during hot reload
          if (import.meta.env.DEV) {
            console.warn("[Tauri] Unlisten promise rejected (hot reload):", error)
          }
        })
    }
  }, [undo, redo])
...
export function ModellerView({ className, onNewProject, onOpenProject }: ModellerViewProps) {
  ...
  // Helper to safely cleanup Tauri listeners during hot reload
  const cleanupTauriListener = useCallback((promise: Promise<() => void> | null) => {
    promise
      ?.then((fn) => {
        try {
          fn()
          ...
          // Callback might already be cleaned up during hot reload
          if (import.meta.env.DEV) {
            console.warn("[Tauri] Failed to unlisten (hot reload):", error)
          }
        }
      })
      .catch((error) => {
  ...
  useEffect(() => {
    let unlistenPromise: Promise<() => void> | null = null
    let unlistenHoverPromise: Promise<() => void> | null = null
    let unlistenCancelledPromise: Promise<() => void> | null = null

    try {
      unlistenPromise = listen<{ paths: string[] }>("tauri://file-drop", (event) => {
      ...
      unlistenHoverPromise = listen("tauri://file-drop-hover", () => {
        setIsDragging(true)
      })

      unlistenCancelledPromise = listen("tauri://file-drop-cancelled", () => {
        setIsDragging(false)
      })
    } catch (error) {
      // During hot reload, listen() might fail - this is expected
      if (import.meta.env.DEV) {
        console.warn("[Tauri] Failed to register file-drop listeners (hot reload):", error)
      }
      return
    }

    return () => {
      cleanupTauriListener(unlistenPromise)
      cleanupTauriListener(unlistenHoverPromise)
      cleanupTauriListener(unlistenCancelledPromise)
    }
  }, [handleFileDrop, cleanupTauriListener])
