...
import { useObjects } from "@/core/stores/modeller"
import { useNavigationStore } from "@/core/stores/navigation-store"
import { shapeIdMap } from "@/lib/hooks/use-kernel"
import {
  createDetailViewRenderContext,
  renderCallout,
  renderCompleteDetailView,
  renderCuttingLine,
} from "@/lib/utils/UT_detail_view"
import {
...
} from "@/lib/utils/UT_dimensions"
import { getModelMetersToDrawingUnitsFactor } from "@/lib/utils/UT_drawing_units"
import {
  getGDTRenderConfig,
  renderCompositeFeatureControlFrame,
  renderDatumFeatureSymbol,
  renderDatumTarget,
  renderFeatureControlFrame,
} from "@/lib/utils/UT_gdt"
import { renderHatchRegion } from "@/lib/utils/UT_hatch"
import {
...
export function Viewport2D({ className, drawingId }: Viewport2DProps) {
  ...
  // Selected view for deletion and other operations
  ...
  const render = useCallback(() => {
    ...
    viewsToRender.forEach((view) => {
      ...
      // UNIT CORRECTION: Detect if projection was generated in model units (meters) instead of paper units (mm)
      // This can happen with projections generated before the unitFactor fix
      // If the view dimensions are very small (< 1 unit), apply a correction factor
      const unitFactor = getModelMetersToDrawingUnitsFactor(drawing.sheetConfig.units)
      const needsUnitCorrection = viewWidth < 1 && viewHeight < 1 && unitFactor > 1
      const coordScale = needsUnitCorrection ? unitFactor : 1

      // Apply correction to view dimensions for rendering
      const scaledViewWidth = viewWidth * coordScale
      const scaledViewHeight = viewHeight * coordScale
      ...
      if (showBoundingBoxes) {
        ...
        ctx.strokeStyle = isDragging
          ? "rgba(74, 222, 128, 0.8)" // Green for dragging
          : isSelected
            ? "rgba(59, 130, 246, 0.9)" // Blue for selected (delete-ready)
            : isHovered
        ...
        ctx.lineWidth = Math.max(
          0.5,
          (isDragging || isSelected ? 1.5 : 0.8) / (paperToScreenScale * viewportZoom)
        )
        ctx.setLineDash([])

        // Bounding box centered on view (use scaled dimensions)
        const halfW = scaledViewWidth / 2
        const halfH = scaledViewHeight / 2
        ctx.strokeRect(-halfW - 3, -halfH - 3, scaledViewWidth + 6, scaledViewHeight + 6)
      }
      ...
      for (const hatch of viewHatches) {
        // Hatch boundary is in view-local coordinates (apply coordScale for consistency)
        renderHatchRegion(ctx, hatch, -viewCenterX * coordScale, -viewCenterY * coordScale)
      }
      ...
      lines.forEach((line: Line2D) => {
        ...
        // Calculate centered coordinates and apply unit correction if needed
        // coordScale corrects for projections generated in model units instead of paper units
        const startX = (line.start.x - viewCenterX) * coordScale
        const startY = -(line.start.y - viewCenterY) * coordScale
        const endX = (line.end.x - viewCenterX) * coordScale
        const endY = -(line.end.y - viewCenterY) * coordScale
      ...
      if (showViewLabels) {
        ...
        // Use scaled dimensions for label positioning
        const halfW = scaledViewWidth / 2
        const halfH = scaledViewHeight / 2
    ...
    // Render GD&T annotations (Feature Control Frames, datum symbols)
    if (drawing.gdt) {
      ctx.save()
      ctx.translate(drawingCenterX, drawingCenterY)
      ctx.scale(paperToScreenScale, paperToScreenScale)

      // Determine theme from canvas theme
      const gdtTheme =
        colors === CANVAS_THEMES.white
          ? "light"
          : colors === CANVAS_THEMES.blueprint
            ? "blueprint"
            : "dark"
      const gdtConfig = getGDTRenderConfig(gdtTheme)

      // Render Feature Control Frames
      drawing.gdt.featureControlFrames.forEach((fcf) => {
        renderFeatureControlFrame(ctx, fcf, gdtConfig.display, 0, 0, 1)
      })

      // Render Composite Feature Control Frames
      drawing.gdt.compositeFrames.forEach((composite) => {
        renderCompositeFeatureControlFrame(ctx, composite, gdtConfig.display, 0, 0, 1)
      })

      // Render Datum Feature Symbols
      drawing.gdt.datumSymbols.forEach((datum) => {
        renderDatumFeatureSymbol(ctx, datum, gdtConfig.display, 0, 0, 1)
      })

      // Render Datum Targets
      drawing.gdt.datumTargets.forEach((target) => {
        renderDatumTarget(ctx, target, gdtConfig.display, 0, 0, 1)
      })

      ctx.restore()
    }

    // Render Detail Views (boundaries and labels)
    if (drawing.detailViews && drawing.detailViews.length > 0) {
      ctx.save()
      ctx.translate(drawingCenterX, drawingCenterY)
      ctx.scale(paperToScreenScale, paperToScreenScale)
      ...
      const detailTheme =
        colors === CANVAS_THEMES.white
          ...
          : colors === CANVAS_THEMES.blueprint
            ? "blueprint"
            : "dark"
      const detailRctx = createDetailViewRenderContext(ctx, 0, 0, 1, detailTheme)

      drawing.detailViews.forEach((detail) => {
        if (detail.visible) {
          renderCompleteDetailView(detailRctx, detail, detail.position)
        }
      })
    ...
    // Render Cutting Lines (section lines)
    if (drawing.cuttingLines && drawing.cuttingLines.length > 0) {
      ctx.save()
      ctx.translate(drawingCenterX, drawingCenterY)
      ctx.scale(paperToScreenScale, paperToScreenScale)
      ...
      const cutTheme =
        colors === CANVAS_THEMES.white
          ...
          : colors === CANVAS_THEMES.blueprint
            ? "blueprint"
            : "dark"
      const cutRctx = createDetailViewRenderContext(ctx, 0, 0, 1, cutTheme)

      drawing.cuttingLines.forEach((cuttingLine) => {
        renderCuttingLine(cutRctx, cuttingLine)
      })
    ...
    // Render Callouts
    if (drawing.callouts && drawing.callouts.length > 0) {
      ctx.save()
      ctx.translate(drawingCenterX, drawingCenterY)
      ctx.scale(paperToScreenScale, paperToScreenScale)
      ...
      const calloutTheme =
        colors === CANVAS_THEMES.white
          ...
          : colors === CANVAS_THEMES.blueprint
            ? "blueprint"
            : "dark"
      const calloutRctx = createDetailViewRenderContext(ctx, 0, 0, 1, calloutTheme)

      drawing.callouts.forEach((callout) => {
        renderCallout(calloutRctx, callout)
      })
  ...
  const handleMouseDown = useCallback(
    (e: React.MouseEvent<HTMLCanvasElement>) => {
      ...
      if (e.button === 2) {
        if (selectedPoints.length > 0) {
          // PERF: console.log("🚫 Cancelled selection")
          setSelectedPoints([])
          e.preventDefault()
        }
      ...
      if (e.button === 0 && !e.ctrlKey && !activeTool) {
        ...
        // Check for view selection/dragging
        const viewHit = findViewAtPoint(paperPoint)

        if (viewHit && drawing) {
          // Clicked on a view
          if (viewHit.view.id === selectedViewId) {
            // Already selected - start dragging if not locked
            if (!viewsLocked) {
            ...
            // Select this view
            setSelectedViewId(viewHit.view.id)
          }
          e.preventDefault()
          return
        }

        // Clicking on empty space - clear view selection and enable pan if locked
      ...
      if (activeTool && e.button === 0 && !e.ctrlKey && drawing) {
        ...
        // PERF: Dimensioning click logging removed
        ...
        if (activeTool === "nota") {
          ...
          if (lineHit) {
            ...
            if (view) {
              ...
              // PERF: console.log("📝 Annotation created!")
              e.preventDefault()
              return
            }
          }
        ...
        if (activeTool === "line-length" && !activeSnap) {
          ...
          if (lineHit) {
            ...
            if (view) {
              ...
              // PERF: console.log("📏 Creating ALIGNED dimension for line-length")
              ...
              // PERF: console.log("✨ Line dimension added!")
              e.preventDefault()
              return
            }
          }
        ...
        if (activeSnap) {
          ...
          // PERF: console.log(`📍 Snapped to ${activeSnap.snap.type.toUpperCase()}:`, pointInPaperCoords)
          ...
          // PERF: console.log("✅ Point selected:", newSelection.length, "points total", pointInPaperCoords)
          ...
          if (newSelection.length >= requiredPoints) {
            // PERF: console.log("🔧 Attempting to create dimension with points:", newSelection)
            ...
            // PERF: console.log("📊 Points for dimension (view-relative):", { p1, p2, anchorViewId })
            ...
            // PERF: Scale info logging removed
            ...
            if (activeTool === "auto") {
              ...
              // PERF: console.log("📏 Auto mode - Angle:", angle)
              ...
              if (
                ...
                // PERF: console.log("→ Creating HORIZONTAL dimension")
              ...
              // PERF: console.log("↗ Creating ALIGNED dimension (line-length/point-to-line)")
              dimension = createAlignedDimension(p1, p2, config, scaleAwareOffset)
            } else if (activeTool === "point-to-point") {
              // Point-to-point: use aligned dimension
              // PERF: console.log("↗ Creating ALIGNED dimension (point-to-point)")
              dimension = createAlignedDimension(p1, p2, config, scaleAwareOffset)
              ...
              // PERF: console.log(`📐 Creating ANGULAR dimension (${measureReflex ? "exterior" : "interior"} angle)`)
            ...
            if (dimension) {
              ...
              // PERF: console.log("📏 Dimension created (attached to view):", dimension)
              addDimension(drawing.id, dimension)
              setSelectedPoints([])
              // PERF: console.log("✨ Dimension added to drawing successfully!")
            } else {
              console.error("❌ Failed to create dimension - dimension is null")
            }
          } else {
            const remaining = requiredPoints - newSelection.length
            // PERF: console.log(`⏳ Waiting for ${remaining} more point(s)`)
          }
        }
  ...
  const handleSelectShapeForDrawing = useCallback(
    (sceneObjectId: string) => {
      ...
      setShowShapeSelector(false) // Go back to orientation selection after selecting
    },
    [drawing]
  )
  ...
  const handleAddView = useCallback(
    async (projectionType: ProjectionType) => {
      ...
      // PRIORITY: If user explicitly selected a shape via the popover, use that
      // Otherwise fall back to the drawing's existing source shapes
      let sceneObjectId: string | undefined

      if (selectedShapeIdForView) {
        // User explicitly selected a shape from the popover
        sceneObjectId = selectedShapeIdForView
        // Update the drawing with this source shape if it's different
        if (!drawing.sourceShapeIds.includes(selectedShapeIdForView)) {
          updateDrawing(drawing.id, {
            sourceShapeIds: [...drawing.sourceShapeIds, selectedShapeIdForView],
          })
        }
      } else {
        // Fall back to existing source shape
        sceneObjectId = drawing.sourceShapeIds[0]
      }
  ...
  return (
    <div className={`relative h-full w-full ${className ?? ""}`}>
      <canvas
        ...
        onKeyDown={(e) => {
          ...
          if (
            ...
            // PERF: console.log("🗑️ Deleting dimension at index:", selectedDimensionIndex)
          ...
          if (
            ...
            // PERF: console.log("🗑️ Deleting annotation:", selectedAnnotationId)
            removeAnnotation(drawing.id, selectedAnnotationId)
            setSelectedAnnotationId(null)
            e.preventDefault()
          }
          // Delete selected view with Delete or Backspace
          if ((e.key === "Delete" || e.key === "Backspace") && selectedViewId !== null && drawing) {
            // PERF: console.log("🗑️ Deleting view:", selectedViewId)
            removeView(drawing.id, selectedViewId)
            setSelectedViewId(null)
            e.preventDefault()
          }
      ...
      {/* Add View Button - always available */}
      {drawing && (
        <Popover
          ...
          onOpenChange={(open) => {
            ...
            if (!open) {
              setSelectedShapeIdForView(null) // Reset selection when closing
              setShowShapeSelector(false) // Reset shape selector when closing
            }
          }}
        >
          <PopoverTrigger asChild>
            {/* Prominent button when no views, smaller button when views exist */}
            {drawing.views.length === 0 ? (
              ...
              <Button
                size="sm"
                variant="outline"
                className="absolute top-4 left-48 bg-background/95 backdrop-blur-sm border-border/50 shadow-sm hover:bg-muted"
                onClick={() => setShowAddViewPopover(true)}
              >
                <HugeiconsIcon icon={Add01Icon} className="size-4 mr-1" />
                {t("drawings.views.addView", "Añadir vista")}
              </Button>
            )}
          </PopoverTrigger>
          <PopoverContent className="w-72 p-2">
            {/* Step 1: Select shape if drawing has no source shapes or user wants to change */}
            {(needsShapeSelection || showShapeSelector) && !selectedShapeIdForView ? (
              <div className="space-y-2">
                ...
                {availableShapesForDrawing.length === 0 ? (
                  ...
                  <>
                    ...
                    {/* Back button when user manually triggered shape selection */}
                    {showShapeSelector && !needsShapeSelection && (
                      <div className="pt-2 border-t border-border/50 mt-2">
                        ...
                        >
                          <HugeiconsIcon icon={ArrowLeft01Icon} className="size-3 mr-1" />
              ...
              <div className="space-y-1">
                {/* Show current source shape or selected shape */}
                {(selectedShapeIdForView || drawing.sourceShapeIds.length > 0) && (
                  <div className="flex items-center justify-between px-2 py-1.5 mb-2 bg-primary/10 rounded text-xs">
                    <span className="text-primary font-medium truncate">
                      {selectedShapeIdForView
                        ? availableShapesForDrawing.find((o) => o.id === selectedShapeIdForView)
                            ?.name
                        : availableShapesForDrawing.find((o) => o.id === drawing.sourceShapeIds[0])
                            ?.name || t("drawings.views.currentShape", "Objeto actual")}
                    </span>
                    {availableShapesForDrawing.length > 1 && (
                      <Button
                        size="sm"
                        variant="ghost"
                        className="h-5 px-1 text-[10px]"
                        onClick={() => {
                          setSelectedShapeIdForView(null)
                          setShowShapeSelector(true)
                        }}
                      >
                        {t("drawings.views.changeShape", "Cambiar")}
                      </Button>
                    )}
                  </div>
                )}
                ...
                {/* Option to select a different shape if multiple shapes available */}
                {availableShapesForDrawing.length > 1 && !selectedShapeIdForView && (
                  <div className="pt-2 border-t border-border/50 mt-2">
                    <Button
                      size="sm"
                      variant="ghost"
                    ...
                    >
                      {t("drawings.views.selectDifferentShape", "Seleccionar otro objeto...")}
                    </Button>
                  </div>
                )}
              </div>
      ...
      {drawing && selectedViewId !== null && (
        <div className="absolute top-4 left-1/2 -translate-x-1/2 flex items-center gap-2 bg-blue-500/20 border border-blue-500/30 rounded-lg px-3 py-2 shadow-lg backdrop-blur-sm">
          <HugeiconsIcon icon={ViewIcon} className="size-4 text-blue-400" />
          <span className="text-sm text-blue-400">
            {t("drawings.views.selected", {
              name:
                drawing.views.find((v) => v.id === selectedViewId)?.projection?.label || "Vista",
            })}
          </span>
          <button
            type="button"
            ...
            className="flex items-center gap-1 px-2 py-1 rounded bg-red-500/20 hover:bg-red-500/30 text-red-400 text-xs transition-colors"
            title={t("drawings.views.delete", "Eliminar vista")}
          >
            <HugeiconsIcon icon={Delete02Icon} className="size-3.5" />
            {t("drawings.views.delete", "Eliminar")}
          </button>
          <span className="text-xs text-muted-foreground">
            ({t("drawings.dimensions.escToDeselect")})
          </span>
