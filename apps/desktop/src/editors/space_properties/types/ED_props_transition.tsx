...
import { TransitionSectionPreview } from "../previews/ED_props_preview_transition"
...
export function TransitionPropertiesPanel({ object, onUpdate }: TransitionPropertiesPanelProps) {
  ...
  // PERFORMANCE: Use getObjectById instead of subscribing to all objects
  const getObjectById = useModellerStore((s) => s.getObjectById)
  const { lengthLabel, convertLengthToDisplay, precision } = useUnits()

  // Get connected channel names (using stable getObjectById function)
  const upstreamChannel = object.upstreamChannelId ? getObjectById(object.upstreamChannelId) : null
  const downstreamChannel = object.downstreamChannelId
    ? getObjectById(object.downstreamChannelId)
    : null
  const upstreamName = upstreamChannel?.name ?? null
  const downstreamName = downstreamChannel?.name ?? null
