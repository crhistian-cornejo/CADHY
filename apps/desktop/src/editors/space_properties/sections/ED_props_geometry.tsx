...
import type { ShapeObject } from "@/core/stores/modeller"
import { PropertyRow } from "@/interface/properties/UI_prop_row"
import { PropertySection } from "@/interface/properties/UI_prop_section"
...
export function GeometrySection({ object, onUpdate }: GeometrySectionProps) {
  ...
  const handleParamChange = useCallback(
    (param: string, value: number) => {
      ...
      // geometry parameters edited - they are complex geometries without simple parameters.
      // Only mesh tessellation quality can be adjusted, which is handled separately.
      if (object.shapeType === "compound") {
        // Note: This is expected behavior for compound shapes, no need to warn
        return
      }
  ...
  const renderParams = () => {
    switch (object.shapeType) {
      ...
      case "compound":
        // Compound shapes don't have editable geometry parameters
        // They are complex geometries from boolean operations, fillets, etc.
        return (
          <div className="text-xs text-muted-foreground italic py-2">
            {t(
              "properties.compoundShapeInfo",
              "This is a compound shape created by boolean operations or modifications. " +
                "Geometry parameters cannot be edited directly."
            )}
          </div>
        )
      default:
