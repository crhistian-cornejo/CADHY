...
import type { OperationParameter, OperationType } from "@/operators/context"
import { BoxCommand, type BoxDialogParams } from "@/operators/interactive"
...
export function useCommand(context: CommandContext) {
  ...
  const executeBoxCommand = useCallback(async (): Promise<string | null> => {
    ...
    // PERF: console.log("[useCommand] Starting box command...")
    ...
    const command = new BoxCommand({
      ...
      onShowDialog: (params: BoxDialogParams) => {
        // PERF: console.log("[useCommand] Showing dialog with params:", params)
        setActiveOperation({
          open: true,
          operation: "extrude",
          title: "Box",
      ...
      onHideDialog: () => {
        // PERF: console.log("[useCommand] Hiding dialog")
    ...
    try {
      ...
      // PERF: console.log("[useCommand] Box command completed, shapeId:", shapeId)
      return shapeId
    } catch (error) {
