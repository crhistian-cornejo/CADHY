...
export async function openExportDialog(
  ...
  const extension = EXPORT_FORMATS.find((f) => f.id === format)?.extension ?? "stl"
...
export async function generateMeshForObject(object: AnySceneObject): Promise<MeshResult | null> {
  switch (object.type) {
    ...
    default:
      // TODO: Add support for other object types (shapes, structures)
      return null
  }
}
...
export async function exportObjects(
  ...
  // Filter to exportable objects (channels and transitions)
  const exportable = objects.filter((o) => o.type === "channel" || o.type === "transition")
  if (exportable.length === 0) {
    return {
      success: false,
      error: "No exportable objects selected. Channels and transitions can be exported.",
    }
  }
