...
import { meshCache } from "@/core/services/mesh-cache"
import type {
  AnySceneObject,
  ChannelObject,
  ChuteObject,
...
import { usePBRTextures } from "@/lib/hooks/use-pbr-textures"
import { generateBoxProjectionUVs } from "../geometry-utils"
import { usePooledMaterial } from "../pool/RE_material_pool"
...
export const SceneObjectMesh = React.memo(function SceneObjectMesh({
  ...
  const color = useMemo(() => {
    ...
    return "#6366f1"
  }, [object, isGhostPreview, isSelected])
  ...
  // PERFORMANCE: Memoize transform arrays to avoid creating new arrays on every render
  // This prevents unnecessary Three.js updates and React reconciliation
  ...
  // Track if textures have been initially applied (to avoid repeated GPU uploads)
  const texturesAppliedRef = useRef<string | null>(null)
  ...
  // Apply textures only when they first load (avoid repeated GPU uploads)
  useEffect(() => {
    if (!objectTextures) {
      texturesAppliedRef.current = null
      return
    }

    // Get texture ID to check if we already applied these textures
    const textureId = objectMaterial?.pbr?.albedoTextureId ?? null
    const alreadyApplied = texturesAppliedRef.current === textureId

    Object.values(objectTextures).forEach((texture) => {
      if (texture) {
        // Always update UV repeat (cheap operation)
        texture.repeat.set(objectUvRepeat.x, objectUvRepeat.y)

        // Only set wrapping and needsUpdate on FIRST application
        if (!alreadyApplied) {
    ...
    // Mark as applied (only log on first application)
    if (!alreadyApplied) {
      texturesAppliedRef.current = textureId
      if (process.env.NODE_ENV === "development") {
        log.log("Textures applied:", Object.keys(objectTextures), "UV repeat:", objectUvRepeat)
      }
    }
  }, [objectTextures, objectUvRepeat, objectMaterial])
  ...
  const geometry = useMemo(() => {
    ...
    // PERFORMANCE: Extract individual parameters instead of JSON.stringify
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [
    object.id,
    object.type,
    object.type === "shape" ? (object as ShapeObject).shapeType : null,
    // Extract individual shape parameters for dependency tracking (avoids JSON.stringify)
    object.type === "shape" ? (object as ShapeObject).parameters.width : null,
    object.type === "shape" ? (object as ShapeObject).parameters.height : null,
    object.type === "shape" ? (object as ShapeObject).parameters.depth : null,
    object.type === "shape" ? (object as ShapeObject).parameters.radius : null,
    object.type === "shape" ? (object as ShapeObject).parameters.bottomRadius : null,
    object.type === "shape" ? (object as ShapeObject).parameters.majorRadius : null,
    object.type === "shape" ? (object as ShapeObject).parameters.minorRadius : null,
    object.type === "shape" ? (object as ShapeObject).parameters.segments : null,
  ...
  // Determine current material configuration
  const materialConfig = useMemo(() => {
    ...
    // Common props for all mesh materials
    const common = {
      color: isGhostPreview ? "#10b981" : isSelected ? "#10b981" : isHovered ? "#3b82f6" : color,
    ...
    if (shadingMode === "workbench") {
      return {
        type: "basic" as const,
        ...common,
        color: isGhostPreview ? "#10b981" : isSelected ? "#6ee7b7" : isHovered ? "#93c5fd" : color,
      }
    }
    ...
    // PBR / Material / Rendered mode
    const isRendered = shadingMode === "rendered"
    const emissiveIntensity = isGhostPreview ? 0.2 : isSelected ? 0.3 : isHovered ? 0.15 : 0

    return {
      type: "standard" as const,
      ...common,
      color:
        isGhostPreview || isSelected || isHovered
          ? common.color
          : objectTextures?.albedo
            ? "#ffffff"
            : color,
      metalness: isRendered
        ? Math.min(1, materialProps.metalness + (viewportSettings.reflection ?? 0) * 0.5)
        : materialProps.metalness,
      roughness: isRendered
        ? Math.max(0, materialProps.roughness - (viewportSettings.reflection ?? 0) * 0.3)
        : materialProps.roughness,
      emissive: common.color,
      emissiveIntensity,
      map: objectTextures?.albedo ?? null,
      normalMap: objectTextures?.normal ?? null,
      roughnessMap: objectTextures?.roughness ?? null,
      metalnessMap: objectTextures?.metalness ?? null,
      aoMap: isRendered ? (objectTextures?.ao ?? null) : null,
      aoMapIntensity: isRendered && objectTextures?.ao ? 1 : 0,
      envMapIntensity: isRendered
        ? objectTextures?.albedo
    ...
    viewportSettings.shadingMode,
    viewportSettings.reflection,
    viewMode,
    isGhostPreview,
    isSelected,
    isHovered,
  ...
  const material = usePooledMaterial(materialConfig)
