...
import type { PBRTextureMaps } from "@/core/services/texture-service"
import { type ChannelObject, useViewportSettings } from "@/core/stores/modeller"
import {
...
export const ChannelMesh = React.memo(function ChannelMesh({
  ...
  // Track if textures have been configured (to avoid repeated GPU uploads)
  const texturesConfiguredRef = useRef<string | null>(null)

  // Apply UV repeat and wrapping to textures (ONLY on first load)
  // PERFORMANCE: needsUpdate triggers synchronous GPU upload (~100-500ms)
  // Parent component (RE_object_mesh) handles initial needsUpdate, we just set UV params
  useEffect(() => {
    if (pbrTextures) {
      // Generate a unique key for this texture set
      const textureKey = pbrTextures.albedo?.uuid ?? "none"
      const isFirstConfig = texturesConfiguredRef.current !== textureKey

      Object.values(pbrTextures).forEach((texture) => {
        if (texture) {
          texture.repeat.set(uvRepeat.x, uvRepeat.y)
          // Only configure wrapping on first load (triggers internal state change)
          if (isFirstConfig) {
            texture.wrapS = THREE.RepeatWrapping
            texture.wrapT = THREE.RepeatWrapping
            // NOTE: needsUpdate is handled by parent - don't set here
          }
        }
      })

      if (isFirstConfig) {
        texturesConfiguredRef.current = textureKey
      }
    }
