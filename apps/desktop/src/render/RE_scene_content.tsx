...
import { useTopologicalSelection, useTopology } from "@/core/kernel"
import { lodManager } from "@/core/services/lod-manager"
import { registerCamera, registerRenderer, registerScene } from "@/core/services/viewport-registry"
import {
  type AnySceneObject,
  type ChannelObject,
  type ShapeObject,
...
import { useCADOperations } from "@/lib/hooks/use-operators"
import { getCameraAtTime } from "@/lib/utils/UT_camera"
import { InteractiveCADOperations } from "@/operators/interactive/OP_interactive_cad"
import { SceneObjectMesh } from "./meshes"
import { CursorTracker } from "./RE_cursor_tracker"
import { PostProcessing, type QualityPreset } from "./RE_postprocessing"
import { AutoSectionPlane } from "./RE_section_plane"
import { TopologicalWireframe } from "./RE_topological_wireframe"
  ...
  return objects.map((obj) => {
    ...
    if (
      ...
      if (objWithMesh.mesh) {
        ...
        // PERF: Debug logging removed for performance
        // if (isCompound || restorationFailed) {
        //   console.log("[HistoryPreview] Restoring mesh for:", objWithMesh.id, {
        //     shapeType, originalVertices, restoredVerticesLength, restoredIndicesLength, restorationFailed
        //   })
        // }
        void isCompound
        void restorationFailed
...
export function SceneContent({ showStats }: SceneContentProps) {
  ...
  // Get invalidate and performance functions for render-on-demand and adaptive quality
  // PERFORMANCE: regress() temporarily lowers quality during camera movement
  // Note: renamed to r3fPerformance to avoid conflict with window.performance
  const { invalidate, performance: r3fPerformance } = useThree()
  ...
  // Invalidate frame when important visual state changes (render-on-demand optimization)
  // This ensures the canvas updates when objects, selection, or settings change
  useEffect(() => {
    invalidate()
  }, [
    invalidate,
  ...
  // When toggling PBR/post-processing or performance mode, force a render and ensure OrbitControls isn't left disabled.
  useEffect(() => {
    invalidate()
  ...
  const handleSelect = useCallback(
    (id: string, additive: boolean) => {
      select(id, additive)
      invalidate() // Trigger re-render for selection highlight
    },
    [select, invalidate]
  )
  ...
  const handleHover = useCallback(
    (id: string | null) => {
      setHovered(id)
      invalidate() // Trigger re-render for hover highlight
    },
    [setHovered, invalidate]
  )

  const handleBackgroundClick = useCallback(() => {
    useModellerStore.getState().deselectAll()
    invalidate() // Trigger re-render for deselection
  }, [invalidate])

  // PERFORMANCE: Memoized callback for OrbitControls to avoid creating new functions on every render
  // Also calls regress() to temporarily lower quality during camera movement for smoother interaction
  const handleCameraChange = useCallback(() => {
    r3fPerformance.regress()
    invalidate()
  }, [invalidate, r3fPerformance])
  ...
  const handleTransformChange = useCallback(() => {
    ...
    // CRITICAL: Invalidate to trigger re-render with frameloop="demand"
    // Without this, the canvas won't update when dragging the transform gizmo
    invalidate()
  ...
  }, [firstSelectedObject, updateObject, invalidate])
  ...
  return (
    <>
      ...
      {/* PERFORMANCE: Skip heavy HDRI in performance mode */}
      {viewportSettings.environmentEnabled && !viewportSettings.performanceMode && (
        <Environment
      ...
      {/* Fallback ambient light when environment is disabled or in performance mode */}
      {(!viewportSettings.environmentEnabled || viewportSettings.performanceMode) && (
        <ambientLight intensity={0.6} />
      )}

      {/* Main Directional Light for Shadows - PERFORMANCE: Skip in performance mode */}
      {viewportSettings.shadows && !viewportSettings.performanceMode && (
        <directionalLight
          position={[15, 20, 10]}
          intensity={1.2}
          castShadow
      ...
      {/* Simple directional light for performance mode (no shadows) */}
      {viewportSettings.performanceMode && (
      ...
      {/* Soft Shadows - PERFORMANCE: Skip in performance mode (very expensive) */}
      {viewportSettings.shadows &&
        viewportSettings.postProcessingQuality !== "low" &&
        !viewportSettings.performanceMode && (
          <ContactShadows
            position={[0, 0.005, 0]}
            opacity={0.5}
      ...
      {/* Post-Processing Effects - PERFORMANCE: Skip in performance mode */}
      {(viewportSettings.enablePostProcessing ?? false) && !viewportSettings.performanceMode && (
        <PostProcessing
