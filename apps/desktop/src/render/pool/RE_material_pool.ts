...
export interface LambertMaterialConfig {
  type: "lambert"
  color?: string | number
  opacity?: number
  transparent?: boolean
 ...
 * This key is used to cache and retrieve materials.
 */
export function generateMaterialKey(config: MaterialConfig): string {
  const parts: string[] = [config.type]

  addCommonKeyParts(config, parts)

  // Extra properties for Standard and Lambert
  if (config.type === "standard" || config.type === "lambert") {
    addExtraKeyParts(config, parts)
  }

  // Type-specific properties
  if (config.type === "standard") {
    addStandardKeyParts(config, parts)
  } else if (config.type === "basic") {
    addBasicKeyParts(config, parts)
...
function addCommonKeyParts(config: MaterialConfig, parts: string[]): void {
  ...
  if (config.transparent !== undefined) parts.push(`t:${config.transparent ? "1" : "0"}`)
}

function addStandardKeyParts(config: StandardMaterialConfig, parts: string[]): void {
...
function addBasicKeyParts(config: BasicMaterialConfig, parts: string[]): void {
  if (config.wireframe) parts.push("w:1")
  if (config.side !== undefined) parts.push(`s:${config.side}`)
}

function addExtraKeyParts(
  config: StandardMaterialConfig | LambertMaterialConfig,
  parts: string[]
): void {
  if (config.wireframe) parts.push("w:1")
  if (config.side !== undefined) parts.push(`s:${config.side}`)
  if (config.emissive !== undefined) parts.push(`e:${config.emissive}`)
  if (config.emissiveIntensity !== undefined) parts.push(`ei:${config.emissiveIntensity}`)
}
...
class MaterialPoolManager {
  ...
  private createMaterial(config: MaterialConfig): THREE.Material {
    switch (config.type) {
      ...
      case "line":
        return this.createLineMaterial(config)
      default:
        throw new Error(`Unknown material type: ${(config as any).type}`)
    }
  }

  private createStandardMaterial(config: StandardMaterialConfig): THREE.MeshStandardMaterial {
  ...
  private createLambertMaterial(config: LambertMaterialConfig): THREE.MeshLambertMaterial {
  ...
  private createBasicMaterial(config: BasicMaterialConfig): THREE.MeshBasicMaterial {
  ...
  private createLineMaterial(config: LineMaterialConfig): THREE.LineBasicMaterial {
...
/**
 * Get a MeshLambertMaterial from the pool
 */
export function getLambertMaterial(
  config: Omit<LambertMaterialConfig, "type">
): THREE.MeshLambertMaterial {
  return materialPool.getMaterial<THREE.MeshLambertMaterial>({ type: "lambert", ...config })
}
...
/**
 * Release a lambert material back to the pool
 */
export function releaseLambertMaterial(config: Omit<LambertMaterialConfig, "type">): void {
  materialPool.releaseMaterial({ type: "lambert", ...config })
}
...
export function usePooledMaterial<T extends THREE.Material>(config: MaterialConfig): T {
  ...
  const key = useMemo(() => generateMaterialKey(config), [config])
  ...
  // Release on unmount or key change
  useEffect(() => {
    return () => {
...
/**
 * Hook for Lambert materials
 */
export function useLambertMaterial(
  config: Omit<LambertMaterialConfig, "type">
): THREE.MeshLambertMaterial {
  return usePooledMaterial<THREE.MeshLambertMaterial>({ type: "lambert", ...config })
}
