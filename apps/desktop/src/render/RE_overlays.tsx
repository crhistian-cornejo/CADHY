...
import { DrawingConfigDialog } from "@/interface/dialogs/UI_drawing_config_dialog"
import { shapeIdMap } from "@/lib/hooks/use-kernel"
import { getModelMetersToDrawingUnitsFactor } from "@/lib/utils/UT_drawing_units"
import { useCADOperationsContext } from "@/operators/context"
import {
  BooleanIcon,
  ControlPointIcon,
  CutIcon,
...
function BottomToolbar() {
  ...
  const handleTogglePerformanceMode = useCallback(() => {
    setViewportSettings({
      performanceMode: !viewportSettings.performanceMode,
    })
    toast.info(
      viewportSettings.performanceMode ? "Performance Mode: OFF" : "Performance Mode: ON (60fps)",
      { duration: 1500 }
    )
  }, [viewportSettings.performanceMode, setViewportSettings])
    ...
    <div className="absolute bottom-3 left-1/2 -translate-x-1/2 flex items-center gap-0.5 p-1 rounded-2xl bg-background/95 backdrop-blur-md border border-border/40 shadow-xl pointer-events-auto">
      ...
      <div className="flex items-center gap-0.5 px-1">
        <ToolbarButton
          side="top"
          icon={FlashIcon}
          label="Performance Mode (60fps)"
          shortcut="Shift+P"
          active={viewportSettings.performanceMode}
          color={viewportSettings.performanceMode ? "text-yellow-500" : undefined}
...
async function createShapeInBackend(obj: AnySceneObject): Promise<string | null> {
  ...
  try {
    ...
    if (result) {
      // DEBUG: console.log(`[Drawing] Created shape "${obj.name}" in backend with ID: ${result.id}`)
