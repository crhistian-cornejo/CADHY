...
export class FrustumCuller {
  ...
  // PERFORMANCE: Reusable Vector3 objects to avoid GC allocations per object
  private readonly _tempWorldPos = new THREE.Vector3()
  private readonly _tempWorldScale = new THREE.Vector3()
  ...
  isObject3DVisible(object: THREE.Object3D): boolean {
    ...
    // PERFORMANCE: Reuse Vector3 instances to avoid GC allocations
    object.getWorldPosition(this._tempWorldPos)
    object.getWorldScale(this._tempWorldScale)

    const maxScale = Math.max(
      this._tempWorldScale.x,
      this._tempWorldScale.y,
      this._tempWorldScale.z
    )
    const worldRadius = geometry.boundingSphere.radius * maxScale

    return this.isSphereVisible(this._tempWorldPos, worldRadius)
  }
}
