...
import type { ObjectType } from "../ST_types"
...
export const createSelectionSlice: StateCreator<ModellerStore, [], [], SelectionSlice> = (
  ...
  // PERFORMANCE OPTIMIZATION: Selection no longer modifies objects array
  // This eliminates N object recreations per selection change
  // Components should use selectedIds.includes(id) instead of obj.selected
  select: (id, additive = false) => {
    set((state) => {
      ...
      return {
        ...
        // REMOVED: objects.map() - no longer update obj.selected
      }
    })
  },
