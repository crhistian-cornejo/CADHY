...
import { captureViewportThumbnailDelayed } from "@/core/services/thumbnail-service"
import { shapeIdMap } from "@/lib/hooks/use-kernel"
import { safeSerialize } from "@/lib/utils/UT_serialization"
...
import type { AnySceneObject } from "./ST_types"
...
export type { ProjectFullData, ProjectInfo, ProjectSettings, ProjectTemplate }
    ...
    (set, get) => ({
      ...
      openExistingProject: async (path) => {
        ...
        try {
          ...
          const batchUpdates: Array<{ id: string; changes: Partial<AnySceneObject> }> = []

          // PERFORMANCE OPTIMIZATION: Parallel batch processing for shape recreation
          // Instead of sequential await (100 shapes × 500ms = 50s),
          // we process in parallel batches (100 shapes / 4 parallel = ~12.5s)
          const shapeObjects = objects.filter((o) => o.type === "shape")
          const BATCH_SIZE = 4 // Process 4 shapes in parallel
          const totalShapes = shapeObjects.length

          log.info(
            `[Project Load] Recreating ${totalShapes} shapes in backend (batch size: ${BATCH_SIZE})...`
          )

          // Helper to process a single shape
          const processShape = async (obj: AnySceneObject) => {
          ...
          // Process shapes in parallel batches
          for (let i = 0; i < shapeObjects.length; i += BATCH_SIZE) {
            const batch = shapeObjects.slice(i, i + BATCH_SIZE)
            const results = await Promise.all(batch.map(processShape))
            ...
            if (i + BATCH_SIZE < shapeObjects.length) {
              await new Promise((resolve) => setTimeout(resolve, 0))
            }

            // Log progress
            const processed = Math.min(i + BATCH_SIZE, totalShapes)
            log.info(`[Project Load] Processed ${processed}/${totalShapes} shapes...`)
          }

          // Apply all updates in a single batch
          if (batchUpdates.length > 0) {
            useModellerStore.getState().updateMultiple(batchUpdates)
          }
      ...
      saveCurrentProject: async () => {
        ...
        try {
          ...
          // Safely serialize data to remove circular references before IPC call
          const safeSceneData = safeSerialize(sceneData)
          const safeDrawingsData = drawingsData ? safeSerialize(drawingsData) : undefined

          const projectInfo = await saveProjectService(
            currentProject.path,
            safeSceneData,
            safeDrawingsData
          )
