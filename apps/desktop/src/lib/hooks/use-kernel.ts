...
import {
  ...
  tessellate,
  translate,
} from "@/core/services/cad-service"
import { type ShapeObject, useModellerStore } from "@/core/stores/modeller"
...
export function useCAD() {
  ...
  const ensureShapeInBackend = useCallback(
    async (sceneId: string): Promise<string | null> => {
      ...
      try {
        // For compound shapes, try to deserialize from BREP data first
        if (obj.shapeType === "compound" && obj.metadata?.brepData) {
          try {
            logger.log("[useCAD] Recreating compound shape from BREP data")
            result = await deserializeShape(obj.metadata.brepData)
            if (result && result.id) {
              shapeIdMap.set(sceneId, result.id)
              logger.log("[useCAD] Compound shape recreated from BREP:", result.id)
              return result.id
            }
          } catch (brepError) {
            logger.error("[useCAD] Failed to deserialize compound shape from BREP:", brepError)
        ...
        switch (obj.shapeType) {
          ...
          case "compound": {
            // Compound shape without BREP data cannot be recreated
            logger.error(
              "[useCAD] Cannot recreate compound shape without BREP data. " +
                "Shape may have been created before BREP serialization was implemented."
            )
            return null
          }
          default:
    ...
    async (sceneId: string, newParameters: Record<string, number>): Promise<boolean> => {
      try {
        ...
        switch (obj.shapeType) {
          ...
          case "compound": {
            // Compound shapes (from boolean operations, fillets, etc.) cannot have their
            // parameters edited - they are complex geometries that don't have simple parameters.
            // Only the mesh tessellation quality (segments) can be adjusted, which is handled
            // separately in the mesh update path.
            logger.warn(
              "[useCAD] Cannot update parameters for compound shape - these are complex geometries " +
                "created by boolean operations or other modifications. Only mesh detail can be adjusted."
            )
            return false
          }
          default:
        ...
        // Tessellate the new geometry (PERF: logging removed)
  ...
  return {
    ...
    importStepShape: useCallback(
      ...
      [addObject, select, convertMeshData, calculateBBoxFromVertices, commitToHistory]
    ),
