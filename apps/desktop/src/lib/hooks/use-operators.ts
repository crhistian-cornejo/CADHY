...
import { useModellerStore, useSelectedObjects } from "@/core/stores/modeller"
import { shapeIdMap } from "./use-kernel"
  ...
  const executeBooleanSubtract = useCallback(async () => {
    ...
    try {
      ...
      try {
        const brepData = await cadService.serializeShape(finalId)
        mergedMetadata.brepData = brepData
        // PERF: Boolean Subtract serialization logging removed
      } catch (serializeError) {
        console.warn(
  ...
  const executeBooleanIntersect = useCallback(async () => {
    ...
    try {
      ...
      try {
        const brepData = await cadService.serializeShape(finalId)
        mergedMetadata.brepData = brepData
        // PERF: Boolean Intersect serialization logging removed
      } catch (serializeError) {
        console.warn(
