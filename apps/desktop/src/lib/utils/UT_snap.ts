...
export interface SnapConfig {
  ...
  nearest: boolean
  /** Enable perpendicular snapping (foot of perpendicular from reference point) */
  perpendicular: boolean
  /** Enable tangent snapping (for circles/arcs) */
  tangent: boolean
  /** Enable quadrant snapping (0°, 90°, 180°, 270° on circles) */
  quadrant: boolean
...
export const TOOL_SNAP_CONFIGS: Record<DimensionToolType, Partial<SnapConfig>> = {
   ...
   * - Perpendicular: For measuring perpendicular distances
   ...
   * - Perpendicular: For perpendicular distances
   * - Tangent/Quadrant: For circle measurements
   ...
   * - Perpendicular: For 90° angle snapping
...
/**
 * Find the foot of perpendicular from a reference point to a line.
 * This is used for perpendicular snap - when you have a starting point
 * and want to find where a perpendicular line would hit an existing line.
 *
 * @param referencePoint - The point from which to drop a perpendicular
 * @param lineStart - Start of the line
 * @param lineEnd - End of the line
 * @returns The perpendicular foot point and whether it lies on the segment
 */
export function perpendicularFoot(
  referencePoint: Point2D,
  lineStart: Point2D,
  lineEnd: Point2D
): { point: Point2D; onSegment: boolean; t: number } {
  const dx = lineEnd.x - lineStart.x
  const dy = lineEnd.y - lineStart.y
  const lengthSq = dx * dx + dy * dy

  if (lengthSq === 0) {
    return { point: lineStart, onSegment: true, t: 0 }
  }

  // Project reference point onto the line (unbounded)
  const t =
    ((referencePoint.x - lineStart.x) * dx + (referencePoint.y - lineStart.y) * dy) / lengthSq
  ...
  return { point: foot, onSegment, t }
}

/**
 * Circle/Arc representation for tangent/quadrant snapping
 */
 ...
 * Returns up to 2 tangent points (or null if point is inside circle).
 *
 * @param externalPoint - Point outside the circle
 * @param circle - The circle to find tangents to
 * @returns Array of tangent points (0, 1, or 2 points)
 */
export function tangentPointsToCircle(externalPoint: Point2D, circle: Circle2D): Point2D[] {
  const dx = externalPoint.x - circle.center.x
  const dy = externalPoint.y - circle.center.y
  const distSq = dx * dx + dy * dy
  const dist = Math.sqrt(distSq)

  // Point is inside or on the circle - no tangent
  if (dist <= circle.radius + 1e-10) {
    return []
  }

  // Distance from external point to tangent point along the line to center
  const tangentDist = Math.sqrt(distSq - circle.radius * circle.radius)

  // Angle from center to external point
  const angleToExternal = Math.atan2(dy, dx)

  // Angle of tangent lines from external point
  const tangentAngle = Math.asin(circle.radius / dist)

  // Two tangent points
  const angle1 = angleToExternal + tangentAngle
  const angle2 = angleToExternal - tangentAngle

  return [
    {
      x: externalPoint.x - tangentDist * Math.cos(angle1),
      y: externalPoint.y - tangentDist * Math.sin(angle1),
    },
    {
      x: externalPoint.x - tangentDist * Math.cos(angle2),
      y: externalPoint.y - tangentDist * Math.sin(angle2),
    },
  ]
}

/**
 * Get quadrant points (0°, 90°, 180°, 270°) on a circle.
 *
 * @param circle - The circle
 * @returns Array of 4 quadrant points
 */
export function quadrantPoints(circle: Circle2D): Point2D[] {
  const { center, radius } = circle
  return [
    { x: center.x + radius, y: center.y }, // 0° (East)
    { x: center.x, y: center.y + radius }, // 90° (North)
    { x: center.x - radius, y: center.y }, // 180° (West)
    { x: center.x, y: center.y - radius }, // 270° (South)
  ]
}

// =============================================================================
// PERPENDICULAR SNAP DETECTION
// =============================================================================

/**
 * Find perpendicular snap points from a reference point to all lines.
 * This is contextual - only valid when the user has already selected
 * a first point and is looking for the second point.
 *
 * @param referencePoint - The first point (origin of perpendicular)
 * @param cursorPoint - Current cursor position (for filtering candidates)
 * @param lines - All lines in the view
 * @param tolerance - Snap tolerance
 * @returns Perpendicular snap points near the cursor
 */
export function findPerpendicularSnaps(
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const foot = perpendicularFoot(referencePoint, line.start, line.end)

    // Only consider if the foot is on the segment
    if (!foot.onSegment) continue

    // Check if cursor is near this perpendicular foot
    const dist = distance(cursorPoint, foot.point)
    if (dist < tolerance) {
      snaps.push({
        point: foot.point,
        type: "perpendicular",
        priority: 1, // Same as endpoint
        sourceLineIndex: i,
      })
    }
  }
...
export interface SnapIndicatorStyle {
  endpoint: { color: string; symbol: SnapSymbol }
  midpoint: { color: string; symbol: SnapSymbol }
  intersection: { color: string; symbol: SnapSymbol }
  nearest: { color: string; symbol: SnapSymbol }
  center: { color: string; symbol: SnapSymbol }
  perpendicular: { color: string; symbol: SnapSymbol }
  tangent: { color: string; symbol: SnapSymbol }
  quadrant: { color: string; symbol: SnapSymbol }
}

export const DEFAULT_SNAP_STYLE: SnapIndicatorStyle = {
  endpoint: { color: "#22c55e", symbol: "square" }, // Green
  midpoint: { color: "#3b82f6", symbol: "triangle" }, // Blue
  intersection: { color: "#f59e0b", symbol: "x" }, // Orange
  nearest: { color: "#8b5cf6", symbol: "circle" }, // Purple
  center: { color: "#ec4899", symbol: "circle" }, // Pink
  perpendicular: { color: "#06b6d4", symbol: "perpendicular" }, // Cyan
  tangent: { color: "#10b981", symbol: "tangent" }, // Emerald
  quadrant: { color: "#f97316", symbol: "diamond" }, // Orange
}
...
export function drawSnapIndicator(
  ...
  switch (symbol) {
    ...
    case "perpendicular":
      ...
      ctx.beginPath()
      // Draw the right angle corner
      ctx.moveTo(x - size * 0.4, y + size * 0.4)
      ctx.lineTo(x - size * 0.4, y - size * 0.1)
      ctx.lineTo(x + size * 0.4, y - size * 0.1)
      ctx.stroke()
      // Small square in corner to indicate 90°
      ctx.strokeRect(x - size * 0.4, y - size * 0.1, size * 0.25, size * 0.25)
      break
