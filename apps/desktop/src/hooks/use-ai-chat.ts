...
${profile.stations.length > 5 ? `  ... (${profile.stations.length - 5} more points)` : ""}`
        ...
        if (finalAction === "modifyShape") {
          ...
          const shapeObj = shape as import("@/stores/modeller").ShapeObject
          const updates: Partial<import("@/stores/modeller").ShapeObject> = {}
          const updatedProps: string[] = []
          ...
          for (const [type, objs] of Object.entries(byType)) {
            ...
            for (const obj of objs) {
              ...
              if (includeDetails) {
                ...
                if (obj.type === "shape") {
                  const shape = obj as import("@/stores/modeller").ShapeObject
        ...
        if (finalAction === "getObjectInfo") {
          ...
          if (obj.type === "shape") {
            ...
            const channel = obj as import("@/stores/modeller").ChannelObject
        ...
        if (finalAction === "booleanUnion") {
          ...
          const shapes: import("@/stores/modeller").ShapeObject[] = []
          for (const id of shapeIds) {
            const obj = getObjectById(id)
            if (!obj || obj.type !== "shape") {
              return `Error: Object "${id}" not found or is not a shape.`
            }
            shapes.push(obj as import("@/stores/modeller").ShapeObject)
          }
        ...
        if (finalAction === "booleanSubtract") {
          ...
          const toolShapes: import("@/stores/modeller").ShapeObject[] = []
          for (const id of toolShapeIds) {
            const obj = getObjectById(id)
            if (!obj || obj.type !== "shape") {
              return `Error: Tool shape "${id}" not found or is not a shape.`
            }
            toolShapes.push(obj as import("@/stores/modeller").ShapeObject)
          }

          const baseShape = baseObj as import("@/stores/modeller").ShapeObject
          const newShape = {
            ...JSON.parse(JSON.stringify(baseShape)),
            name,
            metadata: {
        ...
        if (finalAction === "setLOD") {
          ...
          for (const objectId of objectIds) {
            ...
            const shapeObj = obj as import("@/stores/modeller").ShapeObject
